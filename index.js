...
Fileupload.prototype.handle = function (ctx, next) {
  ...
  if (req.method === "POST" || req.method === "PUT") {
    ...
    if (this.config.authorization && !me) {
      return ctx.done({
        statusCode: 403,
        message: "You're not authorized to upload files."
      });
    }
    ...
    var renameAndStore = function (file) {
      fs.rename(file.path, path.join(uploadDir, file.name), function (err) {
        ...
        storedObject.creationDate = Long.fromNumber(new Date().getTime());
        storedObject.lastModified = Long.fromNumber(storedObject.lastModified);
        storedObject.authorDate = Long.fromNumber(storedObject.authorDate);
    ...
    if (this.config.reqGetAuthorization && !me) {
      return ctx.done(null, {
        statusCode: 403,
        message: "You're not authorized to view files."
      });
    }
    ...
    if (this.config.authorization && !me) {
      return ctx.done({
        statusCode: 403,
        message: "You're not authorized to delete files."
      });
    }
...
Fileupload.prototype.del = function (ctx, next) {
  ...
  var findObj = this.config.authorization ? {
    id: fileId,
    uploaderId: me.id
  } : {
    id: fileId
